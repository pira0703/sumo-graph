...
export default function UsersManager({ users: initialUsers }: { users: UserRow[] }) {
  ...
  async function handleRoleChange(userId: string, newRole: 'admin' | 'paid' | 'user') {
    setLoading(userId)
    setMessage(null)
    try {
                ...
                <tr key={user.id} className="border-b border-stone-800 hover:bg-stone-800/40 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-stone-100 font-medium">{user.name}</div>
                  ...
                  <td className="px-4 py-3">
                    <select
                      ...
                      onChange={(e) => handleRoleChange(user.id, e.target.value as 'admin' | 'paid' | 'user')}
